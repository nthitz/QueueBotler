...
(function() {
  var $, ChatManager, DEBUG, IdleUsers, PMManager, PinManager, TTAPI, addToQueue, addToQueueIfNotInQueue, adminIDs, bot, chatModeOnFor, checkForIdleUsers, checkIn, devMode, doQueueActionIfInQueue, getQueueMessages, host, http, init, lastIdleUserCheck, lastSong, latestQueue, logPlay, logSong, logSongInList, makeNameQueueSafe, masterPin, masterPinEnabled, masterPinRemove, modIDs, numberToEmoji, parsePM, pmHelp, processQueueHTML, profiles, querystring, queueLineID, redis, redisClient, removeIdleUsers, removeQueuedPerson, requestPin, requestPinArray, requestQueue, requestQueueTimeout, savePin, savePinInQueue, sendQueueInChat, sendQueueInChatIfVerified, sendQueueInPM, snaggedList, songLog, songLogHost, songLogPath, updateStatus, util, warnedIdleUserAt;
  ...
  removeIdleUsers = function(pins, queue) {
    var diff, idleTime, idleUser, idleUsers, mins, msg, now, pin, pinO, queuePerson, timeParts, validLineID, validLineIDs, warnUser, warnUsers, warningTime, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _len5, _len6, _m, _n, _o;
    validLineIDs = [];
    ...
    for (_l = 0, _len3 = warnUsers.length; _l < _len3; _l++) {
      warnUser = warnUsers[_l];
      pinO = null;
      for (_m = 0, _len4 = pins.length; _m < _len4; _m++) {
        pin = pins[_m];
        if (pin.lineID === warnUser.lineID) {
          pinO = pin;
          break;
        }
      }
      if (pinO !== null) {
        ...
        if ((typeof warnedIdleUserAt[pin.userid] === 'undefined') || (diff > (60 * 1000 * 30))) {
    ...
    for (_n = 0, _len5 = idleUsers.length; _n < _len5; _n++) {
      idleUser = idleUsers[_n];
      if (validLineIDs.indexOf(idleUser.lineID) !== -1) {
        if (pins.length > 0) {
          pinO = null;
          for (_o = 0, _len6 = pins.length; _o < _len6; _o++) {
            pin = pins[_o];
  ...
  requestQueue = function(callback) {
    var cb, queueOptions, randTime, req;
    clearTimeout(requestQueueTimeout);
    requestQueueTimeout = setTimeout(function() {
  ...
  logPlay = function(data, numActive, numListeners) {
    var cb, logData, queueOptions, realScore, req, score;
    lastSong = data.room.metadata.current_song;
    songLog = data.room.metadata.songlog;
    if (typeof process.env.LOGKEY === 'undefined') {
      return;
    }
    ...
    cb = function(response) {
      ...
      return response.on('end', function() {
        ...
        console.log(str);
        if (str.indexOf('needsong' === 0)) {
          parts = str.split(':');
          songid = parts[1];
          if (typeof songid !== 'undefined') {
            return logSongInList(songid);
          }
        } else if (str !== '') {
  ...
  logSongInList = function(songid) {
    var song, _i, _len;
    for (_i = 0, _len = songLog.length; _i < _len; _i++) {
      song = songLog[_i];
      if (song._id === songid) {
        logSong(song);
        return;
    ...
    return console.log(songid + ' not found in song log, seems like a bug');
  };

  logSong = function(song) {
    var cb, queueOptions, req, songData;
  ...
  init = function() {
    ...
    bot.on('ready', function(data) {
      return bot.roomRegister(process.env.ROOMID, function(data) {
        return modIDs = data.room.metadata.moderator_id;
      });
    });
    bot.on('speak', function(data) {
      ...
      if (lower.indexOf('dance') !== -1) {
        return bot.vote('up');
      }
    });
    bot.on('pmmed', function(data) {
      var ignore;
      ignore = ["5267706beb35c101cef63007"];
      if (ignore.indexOf(data.senderid) !== -1) {
        return;
      }
    ...
    bot.on('snagged', function(data) {
      ...
      if (snaggedList.indexOf(data.userid === -1)) {
        return snaggedList.push(data.userid);
      }
    });
    bot.on('update_votes', function(data) {
      var userid;
      if (typeof data.room.metadata.votelog[0][0] !== 'undefined') {
        userid = data.room.metadata.votelog[0][0];
        if (userid.length === 24) {
          return IdleUsers.logUserAction(userid);
        }
      }
    });
    return bot.on('registered', function(data) {
      var byeMsg, userid;
      userid = data.user[0].userid;
      if (userid === '50c3a443eb35c159cdd0a837') {
        return;
      }
      ...
      return PMManager.queuePMs(byeMsg, userid);
    });
  };
