...
(function() {
  ...
  pmHelp = function(msg, userid) {
    ...
    if (msg === "help") {
      ...
      msgs = ["remove: removes you from the queue", "only works if you added with the bot", "aliases: remove, rm, r"];
    } else if (msg === "checkin") {
      msgs = ["checkin: checks you in to the queue", "only works if you added with the bot", "aliases: checkin, check in, ci, c"];
    } else if (msg === "queue") {
      ...
      msgs = ["The Drop Bot", "Created by @nthitz - nthtiz AT gmail DOT com"];
    } else if (msg === 'status') {
  ...
  init = function() {
    ...
    bot.on('pmmed', function(data) {
      if (data.senderid === process.env.USERID) {
        return;
      }
    ...
    return bot.on('registered', function(data) {
      console.log('regged');
      console.log(data);
      return PMManager.queuePMs(["Welcome to March to the Madness hosted by The Drop, BreeBotJr and kroydeveloper. Check the room info for artist information. Have fun!"], data.user[0].userid);
    });
  };
